...
const LEGACY_POLICY_HASH = 'eb744436bf76a7cc18e32b06734b5478';
const D3_POLICY_HASH = 'ce50cde59a1bd0116a593f0db805e1d8';

export const POST_D2_MIGRATIONS = [
...
export const D3_MIGRATION = ['20260904120000', 'batch_5f_d3_domain_rls_cutover'] as const;
...
const D3_FUNCTIONS = [
  ['private.purplelok_can_reference_members(uuid,text,uuid[])', true, true, 'boolean', 's', 'row_security=off'],
  ['private.purplelok_protect_domain_update()', false, false, 'trigger', 'v', 'row_security=on'],
] as const;
...
function requireCondition(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`Batch 5F-D3 contract failed: ${message}`);
}

function validateFunction(actual: FunctionSnapshot, expected: readonly [string, boolean, boolean, string, string, string]): void {
  const [signature, securityDefiner, authenticated, result, volatility, rowSecurity] = expected;
...
export function validateD3Snapshot(snapshot: D3Snapshot, phase: D3Phase): void {
  const expectedMigrations = phase === 'pre' ? POST_D2_MIGRATIONS : [...POST_D2_MIGRATIONS, D3_MIGRATION];
  ...
  const expectedPolicy = phase === 'pre'
    ? { count: 84, hash: LEGACY_POLICY_HASH, d3Named: 0 }
    : { count: 52, hash: D3_POLICY_HASH, d3Named: 52 };
  ...
  for (const [signature, securityDefiner, authenticated, result, rowSecurity] of D1_HELPERS) {
    const helper = snapshot.d1Helpers.find((candidate) => candidate.signature === signature);
    requireCondition(helper, `${signature} is absent`);
    validateFunction(helper, [signature, securityDefiner, authenticated, result, 's', rowSecurity]);
  }
  ...
  for (const signature of D2_FUNCTIONS) {
    ...
    validateFunction(functionState, [signature, true, false, 'trigger', 'v', 'row_security=off']);
  }
  ...
  if (phase === 'pre') {
    requireCondition(snapshot.d3Functions.length === 0 && snapshot.d3Triggers.length === 0, 'D3 objects partially exist');
  } else {
  ...
  requireCondition(snapshot.d4Objects === 0, 'D4 object exists');
}

function phaseFromArgs(): D3Phase {
  const value = process.argv[2];
  if (value !== 'pre' && value !== 'after') throw new Error('Usage: batch5f-d3-production-check.ts <pre|after>');
  return value;
}
...
function directDatabaseUrl(): string {
  ...
  let parsed: URL;
  try { parsed = new URL(value); } catch { throw new Error('Batch 5F-D3 contract failed: database secret is not a valid URL'); }
...
async function verifiedToken(variable: string, expectedUserId: string): Promise<VerifiedToken> {
  ...
  try { claims = JSON.parse(payloadText) as Record<string, unknown>; } catch { throw new Error(`Batch 5F-D3 contract failed: ${variable} JWT payload is malformed`); }
...
async function functionSnapshots(client: Client, names: readonly string[]): Promise<FunctionSnapshot[]> {
  const result = await client.query<{ signature: string; owner: string; volatility: string; security_definer: boolean; result: string; config: string[] | null; execute_grantees: string[] }>(`
    select namespace.nspname||'.'||procedure.proname||'('||replace(pg_catalog.oidvectortypes(procedure.proargtypes),', ', ',')||')' signature,
      ...
      pg_catalog.pg_get_function_result(procedure.oid) result,coalesce(procedure.proconfig,'{}'::text[]) config,
...
async function collectSnapshot(client: Client, tokens: { owner: VerifiedToken; demo: VerifiedToken }): Promise<D3Snapshot> {
  ...
  const d4 = await one<{ count: number }>(client, `with expected(signature) as(values ('private.purplelok_insert_activity(uuid,text,text,uuid,text,jsonb)'),('private.purplelok_protect_payment_insert()'),('public.record_payment(uuid,numeric,text,text)'),('public.send_quote(uuid)'),('public.approve_quote(uuid)'),('public.convert_quote_to_invoice(uuid,text,date,date)'),('public.convert_quote_to_project(uuid)'),('public.change_lead_stage(uuid,text)')) select ((select count(*) from expected where to_regprocedure(signature) is not null)+(select count(*) from information_schema.columns where table_schema='public' and table_name='projects' and column_name='source_quote_id')+(select count(*) from pg_catalog.pg_constraint where conname='projects_source_quote_organization_fkey')+(select count(*) from pg_catalog.pg_class relation join pg_catalog.pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='public' and relation.relname=any(array['invoices_quote_id_unique','payments_invoice_reference_unique','projects_source_quote_id_unique']))+(select count(*) from pg_catalog.pg_trigger where not tgisinternal and tgname='payments_require_protected_workflow'))::integer count`);
  return {
    ...
    d3Triggers: triggerRows.filter(({ name }) => name.startsWith('domain_')), d4Objects: d4.count,
  };
}
...
async function main(): Promise<void> {
  ...
  try { addresses = await resolve6(hostname); } catch { throw new Error('Batch 5F-D3 contract failed: production direct hostname has no IPv6 resolution'); }
  requireCondition(addresses.length > 0, 'production direct hostname has no IPv6 address');
  const client = new Client({ connectionString: databaseUrl, application_name: `batch-5f-d3-${phase}-readonly-check` });
  try {
    ...
    validateD3Snapshot(snapshot, phase);
...
if (import.meta.url === entry) {
  void main().catch((error: unknown) => {
    console.error(error instanceof Error ? error.message : 'Batch 5F-D3 production check failed');
    process.exitCode = 1;
  });
}
