...
import { Lock, Mail, User as UserIcon, ArrowRight, Shield, Sparkles, CheckCircle2 } from 'lucide-react';
    ...
    <div className="min-h-screen flex bg-canvas">
      <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-surface border-r border-line">
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div className="space-y-6">
            ...
            <h1 className="text-3xl font-bold leading-tight tracking-tight text-primary">The enterprise CRM<br />built for <span className="text-purple-700">PURPLELOK</span></h1>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12 bg-canvas">
        <div className="w-full max-w-sm animate-slide-up">
          ...
          <h2 className="text-xl font-bold tracking-tight text-primary mb-1">{mode === 'login' ? 'Welcome back' : mode === 'signup' ? 'Create your account' : 'Reset password'}</h2>
          <p className="text-sm text-secondary mb-6">{mode === 'login' ? 'Sign in to access your command center' : mode === 'signup' ? 'Join the PURPLELOK team' : "We'll send you a reset link"}</p>
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'signup' && <Input label="Full Name" placeholder="Jane Doe" value={fullName} onChange={(e) => setFullName(e.target.value)} icon={<UserIcon size={15} />} required />}
            <Input label="Email" type="email" placeholder="you@purplelok.com" value={email} onChange={(e) => setEmail(e.target.value)} icon={<Mail size={15} />} required />
            {mode !== 'forgot' && <Input label="Password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} icon={<Lock size={15} />} required />}
            {error && <div className="text-sm text-danger bg-red-50 border border-red-100 rounded-lg px-3 py-2">{error}</div>}
            ...
            {mode === 'login' && (
              <div className="flex items-center justify-between text-sm">
                <label className="flex items-center gap-2 cursor-pointer text-secondary select-none">
                  <input type="checkbox" checked={remember} onChange={(e) => setRemember(e.target.checked)} className="w-4 h-4 rounded border-line accent-purple-600" />Remember me
                </label>
            ...
            <Button type="submit" className="w-full" size="lg" loading={loading}>
              {mode === 'login' ? 'Sign in' : mode === 'signup' ? 'Create account' : 'Send reset link'}<ArrowRight size={15} />
            </Button>
          </form>
          <div className="mt-6 text-center text-sm text-secondary">
            {mode === 'login' ? (<>Don't have an account? <button onClick={() => setMode('signup')} className="text-purple-700 hover:underline font-medium">Sign up</button></>) :
             mode === 'signup' ? (<>Already have an account? <button onClick={() => setMode('login')} className="text-purple-700 hover:underline font-medium">Sign in</button></>) :
